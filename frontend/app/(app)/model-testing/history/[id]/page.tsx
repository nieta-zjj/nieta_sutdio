...
export default function TaskDetailPage() {
  ...
  const generateTableData = useCallback(() => {
    ...
    const findCoordinateUrl = (
      targetCoords: Record<string, number>
    ): {
      url: string;
      isError: boolean;
      errorMessage?: string;
    ...
    if (xVariable && !yVariable) {
      ...
      yVariable.values.forEach((yValue, yIndex) => {
        ...
        xVariable.values.forEach((xValue, xIndex) => {
          ...
          const { url, isError, errorMessage, subtaskId, cellData } =
            findCoordinateUrl(targetCoords);
  ...
  const viewMultipleImagesInModal = (
    urls: string[],
    title: string = "",
            ...
            <CardBody className="space-y-3 pt-0">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {task.processed_images}/{task.total_images} ({task.progress}%)
                  </p>
                </div>
              </div>
                        ...
                        if (varData?.values && varData.values.length > 0) {
                          return (
                            <Select
                              ...
                              placeholder={`选择值`}
                              selectedKeys={
                                currentValueIndex !== null ? [`${currentValueIndex}`] : ["0"]
                              }
                              onSelectionChange={(keys) => {
                                const keysArray = Array.from(keys);
                            ...
                            >
                              {varData.values.map((value, index) => (
                                <SelectItem key={`${index}`}>{value.value}</SelectItem>
                              ))}
                            </Select>
                          );
                        }
            ...
            <CardBody>
              <div className="text-center p-8">
                ...
                <p className="text-default-500 mb-4">
                  从coordinates_by_indices中无法找到匹配的坐标数据
                </p>
            ...
            <>
              ...
              <ModalBody className="p-4 overflow-auto">
                <div className="flex gap-4 h-full">
                  {/* 图片显示区域 */}
                  <div
                    className="flex-1 bg-default-900 rounded-lg p-4 flex items-center justify-center"
                  ...
                  <div className="w-96 flex flex-col gap-4">
                      ...
                      <Card>
                        ...
                        <CardBody className="pt-0 space-y-3">
                          ...
                          {yAxis && currentCellData.yValue && (
                            <div>
                              <p className="text-sm text-default-500">
                                Y轴 ({names[yAxis] || yAxis})
                              </p>
                              <p className="text-sm font-medium">{currentCellData.yValue}</p>
                            </div>
                          )}
                          {currentCellData.coordinates &&
                            Object.keys(currentCellData.coordinates).length > 0 && (
                              <div>
                                <p className="text-sm text-default-500 mb-2">完整坐标</p>
                                <div className="space-y-1">
                                  {Object.entries(currentCellData.coordinates).map(
                                    ([dim, value]) => (
                                      <div
                                        key={dim}
                                        className="flex justify-between items-center text-xs bg-default-100 p-2 rounded"
                                      >
                    ...
                    <Card>
                      ...
                      <CardBody className="pt-0">
                        <StarRating rating={currentRating} onRatingChange={saveRating} />
                      </CardBody>
                    </Card>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {currentEvaluations.slice(0, 3).map((evaluation, index) => (
                              <Chip key={index} size="sm" variant="flat">
                                {evaluation.length > 20
                                  ? `${evaluation.substring(0, 20)}...`
                                  : evaluation}
                              </Chip>
                            ))}
                  ...
                  {currentEvaluations.length > 0 ? (
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {currentEvaluations.map((evaluation, index) => (
                        <div
                          key={index}
                          className="flex items-center gap-2 p-3 bg-default-50 rounded-lg"
                        >
